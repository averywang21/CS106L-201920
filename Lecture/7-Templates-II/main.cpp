...
template <typename T>
pair<T, T> my_minmax(const T& num1) {
    return make_pair(num1, num1);
}

//template <typename T, typename ...Ts>
//pair<T, T> my_minmax(const T& num1, const Ts&... args) {
...
template <typename T, typename ...Ts>
pair<T, T> my_minmax(const T& num1, const Ts& ...args) {
    vector<T> arguments{num1, args...};
    T min = arguments[0], max = arguments[0];
...
//template <typename T>
//pair<T, T> my_minmax(T num1, T num2) {
...
int main() {
    ...
    auto[min, max] = my_minmax(-3.48, 28.38, 0.384,-0.2947);
